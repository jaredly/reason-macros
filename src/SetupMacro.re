...
let rec getStringAttribute = (attributes, name) =>
  switch (attributes) {
  | [] => None
  | [
      (
        ...
        PStr([
          {
            pstr_desc:
              Pstr_eval(
                {pexp_desc: Pexp_constant(Pconst_string(v, _))},
                _,
              ),
          },
        ]),
...
let rec collectArgs = expr => {
  ...
  | Pexp_fun(_, _, _, _) =>
    fail(
      expr.pexp_loc,
      "Macro definition functions can't have default values or labels",
    )
  | _ => ([], expr)
  };
};
...
let getName = (pattern, attributes) => {
  ...
  | Ppat_any =>
    switch (getStringAttribute(attributes, "macro.name")) {
    | None =>
      fail(
        pattern.ppat_loc,
        "macro.name must be provided when the let pattern is _",
      )
    | Some(name) => name
    }
  | Ppat_var({txt}) => txt
  | _ =>
    fail(
      pattern.ppat_loc,
      "macro.let pattern must either be a var or _ with macro.name supplied",
    )
  };
};
