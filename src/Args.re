  ...
  | {
    ...
    | "longCapIdent" =>
      ...
      | _ =>
        fail(
          pat.ppat_loc,
          "Argument must be a (possibly namespaced) capitalized Identifier",
        )
      }
    | "capIdent" =>
      switch (pat.ppat_desc) {
      ...
      | _ =>
        fail(
          pat.ppat_loc,
          "Argument must be a non-namespaced capitalized Identifier",
        )
      }
    | "ident" =>
      switch (pat.ppat_desc) {
  ...
  | {
    ...
    | "pattern" =>
      ...
      | _ =>
        fail2(
          expr.pexp_loc,
          "Argument must be a pattern (pass in with [%pat? SomePattern])",
          typloc,
          "Declared here",
        )
      }
    ...
    | "longIdent" =>
      ...
      | _ =>
        fail2(
          expr.pexp_loc,
          "Argument must be a (possibly namespaced) identifier",
          typloc,
          "Declared here",
        )
      }
    | "bool" =>
      ...
      | _ =>
        fail2(
          expr.pexp_loc,
          "Argument must be a boolean literal",
          typloc,
          "Declared here",
        )
      }
...
let setupLocals = (args, payload, loc): locals => {
  switch (args, payload) {
  | ([pattern], PStr([{pstr_desc: Pstr_eval(expr, _)}])) =>
    bindLocal(pattern, expr)
  | (
      args,
      PStr([
        {
          pstr_desc: Pstr_eval({pexp_desc: Pexp_tuple(items), pexp_loc}, _),
        },
      ]),
    ) =>
    let rec loop = (args, items) =>
      switch (args, items) {
      | ([arg, ...args], [item, ...items]) =>
        bindLocal(arg, item) @ loop(args, items)
      | ([], []) => []
      | _ => assert(false)
      };
